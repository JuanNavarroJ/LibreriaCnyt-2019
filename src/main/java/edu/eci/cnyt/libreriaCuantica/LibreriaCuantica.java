...
public class LibreriaCuantica {
     ...
     * Metodo que permite el calculo de un sistema deterministico clasico segun
     * la cantidad de clicks.
     *
     * @param matrizBool Es la matriz booleana que contiene las relaciones entre
     * los vertices.
     * @param estado Es el estado inicial de cada vertice cuantas bolas tiene.
     * @param cantClicks Es la cantidad de clicks que indica los movimientos de
     * las bolas.
     ...
     * Metodo que permite el calculo de un sistema probabilistico segun la
     * cantidad de clicks.
     *
     * @param matrizProb Es la matriz de probabilidades.
     * @param estado Es las probabilidades iniciales.
     * @param cantClicks Son la cantida de estados T que el usuario quiere
     * simular.
     * @return Retorna el estado de las probabilidades segun la cantidad de
     * clicks.
     */
     ...
     * Metodo que permite el calculo de la matriz de probabilidades del
     * experimento de doble rendija segun la cantidad de rendijas igresadas por
     * el usuario.
     *
     * @param cantRendijas Son la cantidad de rendijas iniciales que el usuario
     * desea conocer su probabilidad.
    ...
    /**
     * Programming Drill 4.1 - Quantum Computing for Computer Scientists. Metodo
     * que permite calcular la probabilidad de encontra una particula en una
     * posición en particular.
     *
     * @param estado Es el estado inicial de la particula.
     * @param posicion Es la posicion en donde queremos encontrar la
     * probabilidad en esa posicion.
     * @return
     */
    ...
    /**
     * Programming Drill 4.1 - Quantum Computing for Computer Scientists. Metodo
     * que permite calcular la probabilidad de transitar de un vector a otro.
     *
    ...
    public static NumeroComplejo calcularLaMedia(NumeroComplejo[][] matr, NumeroComplejo[] ket) {
        NumeroComplejo media = null;
        if (Libreria.esHermitian(matr)) {
            NumeroComplejo[] res = Libreria.accionDeUnaMatrizSobreUnVector(matr, ket);
            media = Libreria.productoInterno(res, ket);
        }
        return media;
    }

    public static NumeroComplejo calcularVarianza(NumeroComplejo[][] observable, NumeroComplejo[] ket) {
        NumeroComplejo varianza = null;
        if (Libreria.esHermitian(observable)) {
            NumeroComplejo media = calcularLaMedia(observable, ket);
            NumeroComplejo[][] identidad = Libreria.productoEscalar(observable, media);
            NumeroComplejo[][] temp1 = Libreria.sumaMatricesComplejas(observable, Libreria.inversoMatrizCompleja(identidad));
            NumeroComplejo[][] temp2 = Libreria.productoEntreMatrices(temp1, temp1);
            varianza = calcularLaMedia(temp2, ket);
        }
        return varianza;
    }
}
